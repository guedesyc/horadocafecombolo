...
function ProductCard({ category, flavor, onAdd }: { category: Category; flavor: string; onAdd: (item: Omit<CartItem, 'key'>) => void }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-content">
      ...
      <div className={`size-area ${quantity ? 'is-enabled' : ''}`}><div className="size-label"><span>{isSingleOption ? 'Opção' : 'Escolha o tamanho'}</span>{!quantity && <small>Selecione a quantidade primeiro</small>}</div><div className="size-grid">{category.sizes.map((size) => <button type="button" key={size.label} disabled={!quantity} className={selectedSize?.label === size.label ? 'selected' : ''} onClick={() => setSelectedSize(size)}><span>{size.label}</span><strong>{money(size.price)}</strong></button>)}</div></div>
